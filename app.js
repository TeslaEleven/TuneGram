  ...
  var tags = ["special", "edition", "mix", "demo", "video", "deluxe", "definitive", "live", "acoustic", "explicit", "master", "remix", "version", "bonus", "original", "soundtrack", "remastered", "reissue", "re-recorded", "re-recording", "reimagined", "reimagining", "cover", "stripped down"];
  var plistWarns = [];
  ...
  function resetLyricVariables() {
    ...
    document.querySelectorAll('.lyric-break-note').forEach(note => {
      ...
      note.style.transition = 'all 0.8s cubic-bezier(0.4, 0, 0.2, 1)';
    });
  ...
  function runLyrics(data) {
    ...
    if (data.instrumental == false && (data.plainLyrics || data.syncedLyrics)) {
      if (data.syncedLyrics) {
        ...
        const isFirstInQueue = music.queue.position === 0;
        ...
        function animateLyricTransition(newIndex, oldIndex) {
          ...
          lines.forEach((line, index) => {
            ...
            if (distance <= maxDistance) {
              ...
              const translateY = (index - newIndex) * 8;
        ...
        function scrollParentToChild(parent, child) {
          ...
          const targetScroll = childTopRelativeToParent - (parent.clientHeight / 2) + (child.clientHeight / 2);
        ...
        function updateLyricsHighlight() {
          ...
          lyricLines.forEach((line, index) => {
            const lineTime = parseTime(line.getAttribute('data-time'));
            const nextLineTime = index < lyricLines.length - 1 ?
              parseTime(lyricLines[index + 1].getAttribute('data-time')) :
              lineTime + 2000;

            if (line.classList.contains('lyric-break')) {
              const breakNote = line.querySelector('.lyric-break-note');
              if (breakNote) {
                if (index === newHighlightIndex) {
                  breakNote.style.opacity = '1';
                } else if (currentTime >= lineTime && currentTime < nextLineTime) {
                  const progress = (currentTime - lineTime) / (nextLineTime - lineTime);
                  const opacity = Math.min(0.3 + (progress * 0.7), 1);
                  breakNote.style.opacity = opacity.toString();
                  breakNote.style.filter = `brightness(${1 + (progress * 0.5)})`;
                } else {
                  breakNote.style.opacity = '0.3';
                  breakNote.style.filter = 'brightness(1)';
                }
              }
            }
          });

          if (newHighlightIndex !== window.currentHighlightIndex) {
            lyricLines.forEach(line => {
              line.classList.remove('highlight');
            });
            ...
            if (newHighlightIndex >= 0 && lyricLines[newHighlightIndex]) {
              ...
              setTimeout(() => {
                const parentEl = (typeof lyricsDiv !== 'undefined' && lyricsDiv) ? lyricsDiv : document.getElementById('tlcontent-np') || lyricLines[newHighlightIndex].parentElement;
                scrollParentToChild(parentEl, lyricLines[newHighlightIndex]);
              }, 100);
            } else {
              lyricLines.forEach(line => {
                line.style.transition = 'all 0.3s ease';
            ...
            window.currentHighlightIndex = newHighlightIndex;
          }
        }
